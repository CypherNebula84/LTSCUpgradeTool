...
echo     - possibly system backup before running this script
echo.
echo ISO LINKS:
...
for %%K in EditionID ProductName CurrentBuild CurrentBuildNumber ReleaseId DisplayVersion InstallationType ProductId (
    :: Get old value from current PC
    ...
    :: Print change (skip ProductId)
    if NOT "%%K"=="ProductId" echo Changing %%K from "!oldval!" to "!newval!"
    :: Apply change
